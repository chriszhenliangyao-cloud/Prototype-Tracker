...
function PlanEditor({ rows, months, onClose, onSave }: { rows: PlanningRow[]; months: string[]; onClose: () => void; onSave: (rows: PlanningRow[], publish: boolean, logs: ChangeLog[]) => void }) {
	...
	return <div className="sip-overlay"><section className="sip-modal wide sip-v2-editor" role="dialog" aria-modal="true"><header><div><h2>Rolling Plan Editor</h2><p>Edit rolling monthly forecast and supply plan</p></div><button onClick={onClose}>×</button></header><div className="sip-v2-editor-meta"><div><span>▣ Plan Cycle: {monthLabel(months[0])}</span><span>▣ Window: {shortMonth(months[0])} – {shortMonth(months.at(-1)!)} {months[0].slice(0, 4)}</span><span className="autosaved">◉ Autosaved 2 min ago</span></div><div><b>Draft v4</b><small>Based on {monthLabel(lastMonth(months[0]))} archive</small></div></div><nav className="sip-v2-tabs"><button className={tab === "grid" ? "active" : ""} onClick={() => setTab("grid")}>Grid Input</button><button className={tab === "versions" ? "active" : ""} onClick={() => setTab("versions")}>Version History</button><button className={tab === "log" ? "active" : ""} onClick={() => setTab("log")}>Change Log ({logs.length})</button></nav>
		{tab === "grid" && <><section className="sip-v2-editor-scope"><b>Editable Scope</b><div><span>● Forecast editable: {months.map(shortMonth).join(", ")}</span><span>● Actuals read-only from history</span><span>● Supply Plan editable: {months.map(shortMonth).join(", ")}</span><span>● Current Inventory auto-calculated</span></div></section><div className="sip-v2-editor-tools"><button className="sip-btn">✎ Inline Edit</button><button className="sip-btn" onClick={async () => { const text = await navigator.clipboard.readText(); const values = text.trim().split(/\s+/).map(numberValue); if (values.length >= months.length * 2) setDraft((current) => current.map((row) => row.model === selected ? { ...row, months: { ...row.months, ...Object.fromEntries(months.map((month, index) => [month, { forecast: values[index * 2], supply: values[index * 2 + 1] }])) } } : row)); }}>▣ Paste Grid</button><button className="sip-btn" onClick={() => fileRef.current?.click()}>▧ Import Excel</button><input hidden ref={fileRef} type="file" accept=".xlsx,.xls,.csv" onChange={importFile} /><select value={selected} onChange={(event) => setSelected(event.target.value)}>{rows.map((row) => <option value={row.model} key={row.model}>{row.model}</option>)}</select><button className={`sip-btn ${selectedOnly ? "primary" : ""}`} onClick={() => setSelectedOnly((value) => !value)}>◎ Apply to Selected SKU</button></div><div className="sip-table-scroll sip-v2-editor-scroll"><table className="sip-v2-editor-grid"><thead><tr><th>SKU</th><th>Month</th><th>Forecast</th><th>Supply Plan</th><th>Comment</th></tr></thead><tbody>{visibleRows.flatMap((row) => months.map((month) => <tr key={`${row.model}-${month}`}><td><strong>{row.model}</strong></td><td>{shortMonth(month)}</td><td><NumberInput label={`${row.model} ${month} forecast`} value={row.months[month]?.forecast || 0} onChange={(value) => update(row.model, month, "forecast", value)} /></td><td><NumberInput label={`${row.model} ${month} supply`} value={row.months[month]?.supply || 0} onChange={(value) => update(row.model, month, "supply", value)} /></td><td>{commentFor(row, month)}</td></tr>))}</tbody></table></div></>}
		{tab === "versions" && <div className="sip-v2-version-list"><article><b>Draft v4</b><span>Current editable draft · Now</span><button className="sip-btn" onClick={() => setTab("log")}>Compare</button></article><article><b>Plan Snapshot v3</b><span>Published by Ivy · Jul 31, 2026</span><button className="sip-btn" onClick={() => { setDraft(clone(rows)); setTab("grid"); }}>Restore</button></article><article><b>Plan Snapshot v2</b><span>Published by Ivy · Jun 30, 2026</span><button className="sip-btn" onClick={() => setTab("log")}>View Log</button></article></div>}
