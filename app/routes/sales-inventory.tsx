...
function ModelFilter({ rows, selected, onChange }: { rows: PlanningRow[]; selected: string[]; onChange: (models: string[]) => void }) {
	...
	const options = rows.map((row) => row.model).filter((model) => model.toLowerCase().includes(query.toLowerCase()));
	return <label className="sip-v2-field"><span>Model</span><details className="sip-v2-model"><summary>{selected.length ? `${selected.length} Models` : "All Models"}</summary><div>
		...
		{options.map((model) => <label key={model}><input type="checkbox" checked={selected.includes(model)} onChange={() => onChange(selected.includes(model) ? selected.filter((item) => item !== model) : [...selected, model])} />{model}</label>)}
	</div></details></label>;
}
...
function HistoryModal({ history, syncedMonth, onClose }: { history: HistoryRow[]; syncedMonth: string; onClose: () => void }) {
	const rows = history.filter((item) => item.month === syncedMonth);
	return <div className="sip-overlay"><section className="sip-modal wide sip-v2-history" role="dialog" aria-modal="true"><header><div><h2>History · {monthLabel(syncedMonth)}</h2><p>Read-only actual shipment, actual supply and inventory pulled into the planning baseline.</p></div><button onClick={onClose}>×</button></header><div className="sip-table-scroll"><table><thead><tr><th>Model</th><th>Product</th><th>Actual Shipment</th><th>Actual Supply</th><th>Beginning Inventory</th><th>Ending Inventory</th></tr></thead><tbody>{rows.length ? rows.map((item) => <tr key={item.model}><td>{item.model}</td><td>{item.product}</td><td>{formatNumber(item.actualSales)}</td><td>{formatNumber(item.actualSupply)}</td><td>{formatNumber(item.beginningInventory)}</td><td>{formatNumber(item.endingInventory)}</td></tr>) : <tr><td colSpan={6}>No mock actuals for this month.</td></tr>}</tbody></table></div><footer><span>History is read only</span><button className="sip-btn" onClick={onClose}>Close</button></footer></section></div>;
}
...
function PlanEditor({ rows, months, onClose, onSave }: { rows: PlanningRow[]; months: string[]; onClose: () => void; onSave: (rows: PlanningRow[], publish: boolean, logs: ChangeLog[]) => void }) {
	...
	const applySelected = () => { const source = draft.find((row) => row.model === selected); if (!source) return; setDraft((current) => current.map((row) => row.model === selected ? row : { ...row, months: { ...row.months, ...Object.fromEntries(months.map((month) => [month, clone(source.months[month])])) } })); };
	...
	return <div className="sip-overlay"><section className="sip-modal wide sip-v2-editor" role="dialog" aria-modal="true"><header><div><h2>Rolling Plan Editor</h2><p>Edit the current 3-month Shipment Forecast and Supply Plan. Historical actuals remain read only.</p></div><button onClick={onClose}>×</button></header><div className="sip-v2-editor-meta"><span><b>Plan Cycle</b> {monthLabel(months[0])}</span><span><b>Window</b> {shortMonth(months[0])} – {shortMonth(months.at(-1)!)}</span><span><b>Draft</b> v4 · Mock</span></div><nav className="sip-v2-tabs"><button className={tab === "grid" ? "active" : ""} onClick={() => setTab("grid")}>Grid Input</button><button className={tab === "versions" ? "active" : ""} onClick={() => setTab("versions")}>Version History</button><button className={tab === "log" ? "active" : ""} onClick={() => setTab("log")}>Change Log ({logs.length})</button></nav>
		{tab === "grid" && <><div className="sip-v2-editor-tools"><button className="sip-btn">✎ Inline Edit</button><button className="sip-btn" onClick={async () => { const text = await navigator.clipboard.readText(); const values = text.trim().split(/\s+/).map(numberValue); if (values.length >= months.length * 2) setDraft((current) => current.map((row) => row.model === selected ? { ...row, months: Object.fromEntries(months.map((month, index) => [month, { forecast: values[index * 2], supply: values[index * 2 + 1] }])) } : row)); }}>▣ Paste Grid</button><button className="sip-btn" onClick={() => fileRef.current?.click()}>▧ Import Excel</button><input hidden ref={fileRef} type="file" accept=".xlsx,.xls,.csv" onChange={importFile} /><select value={selected} onChange={(event) => setSelected(event.target.value)}>{rows.map((row) => <option value={row.model} key={row.model}>{row.model}</option>)}</select><button className="sip-btn" onClick={applySelected}>Apply to Selected SKU</button></div><div className="sip-table-scroll"><table className="sip-v2-editor-grid"><thead><tr><th rowSpan={2}>Model</th>{months.map((month) => <th colSpan={2} key={month}>{monthLabel(month)}</th>)}</tr><tr>{months.flatMap((month) => [<th key={`${month}-f`}>Forecast</th>, <th key={`${month}-s`}>Supply</th>])}</tr></thead><tbody>{draft.map((row) => <tr key={row.model}><td>{row.model}<small>{row.product}</small></td>{months.flatMap((month) => [<td key={`${month}-f`}><NumberInput label={`${row.model} ${month} forecast`} value={row.months[month]?.forecast || 0} onChange={(value) => update(row.model, month, "forecast", value)} /></td>, <td key={`${month}-s`}><NumberInput label={`${row.model} ${month} supply`} value={row.months[month]?.supply || 0} onChange={(value) => update(row.model, month, "supply", value)} /></td>])}</tr>)}</tbody></table></div></>}
		{tab === "versions" && <div className="sip-v2-version-list"><article><b>Draft v4</b><span>Current editable draft · Now</span><button className="sip-btn">Compare</button></article><article><b>Plan Snapshot v3</b><span>Published by Ivy · Jul 31, 2026</span><button className="sip-btn">Restore</button></article><article><b>Plan Snapshot v2</b><span>Published by Ivy · Jun 30, 2026</span><button className="sip-btn">View Log</button></article></div>}
		{tab === "log" && <div className="sip-table-scroll"><table><thead><tr><th>SKU</th><th>Month</th><th>Field</th><th>Before</th><th>After</th><th>User</th><th>Time</th></tr></thead><tbody>{logs.length ? logs.map((item, index) => <tr key={`${item.model}-${item.month}-${item.field}-${index}`}><td>{item.model}</td><td>{monthLabel(item.month)}</td><td>{item.field}</td><td>{formatNumber(item.before)}</td><td>{formatNumber(item.after)}</td><td>{item.user}</td><td>{item.time}</td></tr>) : <tr><td colSpan={7}>No changes in this draft.</td></tr>}</tbody></table></div>}
...
export default function SalesInventory() {
	...
	useEffect(() => {
		try { const value = localStorage.getItem(STORAGE_KEY); if (value) { const stored = JSON.parse(value) as Workspace; if (stored.rows?.length && stored.months?.length === 3) { setRows(stored.rows); setMonths(stored.months); setHistory(stored.history || []); setLastClosedMonth(stored.lastClosedMonth); setSnapshots(stored.forecastSnapshots?.length ? stored.forecastSnapshots : clone(seedSnapshots)); } } } catch { localStorage.removeItem(STORAGE_KEY); }
		setTheme(document.documentElement.getAttribute("data-theme") || "light"); setLoaded(true);
	}, []);
	useEffect(() => { if (loaded) localStorage.setItem(STORAGE_KEY, JSON.stringify({ rows, months, history, lastClosedMonth, forecastSnapshots: snapshots } satisfies Workspace)); }, [history, lastClosedMonth, loaded, months, rows, snapshots]);
	...
	const focusCell = (item: ExceptionItem) => { const id = `${item.row.model}-${item.month}`; setHighlight(id); document.getElementById(`sip-cell-${id}`)?.scrollIntoView({ behavior: "smooth", block: "center", inline: "center" }); window.setTimeout(() => setHighlight(""), 2200); };
	...
	{historyOpen && <HistoryModal history={history} syncedMonth={lastClosedMonth || "2026-07"} onClose={() => setHistoryOpen(false)} />}
	{archiveOpen && <ForecastArchiveModal snapshots={snapshots} onClose={() => setArchiveOpen(false)} />}
	{editorOpen && <PlanEditor rows={rows} months={months} onClose={() => setEditorOpen(false)} onSave={(next, publish) => { setRows(next); if (publish) { const now = new Date().toISOString(); setSnapshots((current) => [...current, ...next.flatMap((row) => months.map((month) => ({ archiveMonth: months[0], savedAt: now, model: row.model, product: row.product, category: row.category, forecastMonth: month, shipmentForecast: row.months[month]?.forecast || 0, supplyPlan: row.months[month]?.supply || 0 })))]); } setEditorOpen(false); }} />}
