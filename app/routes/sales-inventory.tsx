...
function TrendView({ rows, months, totals, exceptions, highlightedMonth, onFocus }: { rows: PlanningRow[]; months: string[]; totals: Array<{ month: string; demand: number; supply: number }>; exceptions: ExceptionItem[]; highlightedMonth: string; onFocus: (item: ExceptionItem) => void }) {
	...
	return <section className="sip-trend">
		<div className="sip-trend-top">
			<article className="sip-trend-card sip-demand-chart"><header><h2>Monthly Demand vs Supply <span className="sip-info" title="Demand, supply and net gap by month">i</span></h2></header><footer><span><i className="legend demand" /> Demand</span><span><i className="legend supply" /> Supply</span><span><i className="legend gap" /> Net Gap (S − D)</span></footer><div className="sip-bars" role="img" aria-label="Monthly demand, supply and net gap chart from negative twenty thousand to positive twenty thousand units"><div className="sip-bar-y-axis"><b>20K</b><b>10K</b><b>0</b><b>−10K</b><b>−20K</b></div>{totals.map((item) => { const gap = item.supply - item.demand; const negativeGap = Math.max(0, -gap); const eventRow = heatRows.find(({ first }) => first === item.month); const eventGap = eventRow ? (eventRow.row.months[item.month]?.supply || 0) - (eventRow.row.months[item.month]?.forecast || 0) : 0; const eventEoh = eventRow?.endings[item.month] || 0; const eventTone = eventGap < 0 ? "risk" : eventEoh <= 500 ? "watch" : "ok"; return <div className="sip-bar-month" key={item.month}><div className="sip-bar-plot"><div className="sip-bar-positive"><i className="demand" style={{ height: `${Math.min(100, item.demand / barAxisLimit * 100)}%` }}><b>{compact(item.demand)}</b></i><i className="supply" style={{ height: `${Math.min(100, item.supply / barAxisLimit * 100)}%` }}><b>{compact(item.supply)}</b></i></div><div className="sip-gap-bar"><i style={{ height: `${Math.min(100, negativeGap / barAxisLimit * 100)}%` }}><b>{signedCompact(gap)}</b></i></div></div><div className="sip-zero-line" /><strong>{monthLabel(item.month)}</strong>{eventRow && <span className={`sip-trend-fb ${eventTone}`}>{eventTone === "risk" ? "FB!" : eventTone === "watch" ? "FB~" : "FB✓"} {eventRow.row.model}</span>}</div>; })}</div></article>
