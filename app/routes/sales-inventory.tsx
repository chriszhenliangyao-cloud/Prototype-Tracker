...
function TrendView({ rows, months, totals, exceptions, highlightedMonth, onFocus }: { rows: PlanningRow[]; months: string[]; totals: Array<{ month: string; demand: number; supply: number }>; exceptions: ExceptionItem[]; highlightedMonth: string; onFocus: (item: ExceptionItem) => void }) {
	...
	return <section className="sip-trend">
		<div className="sip-trend-top">
			<article className="sip-trend-card sip-demand-chart"><header><h2>Monthly Demand vs Supply</h2><small>D = Demand · S = Supply · Gap = S − D</small></header><div className="sip-bars" role="img" aria-label="Monthly demand, supply and net gap chart">{totals.map((item) => { const gap = item.supply - item.demand; const eventRow = heatRows.find(({ first }) => first === item.month); const eventGap = eventRow ? (eventRow.row.months[item.month]?.supply || 0) - (eventRow.row.months[item.month]?.forecast || 0) : 0; const eventEoh = eventRow?.endings[item.month] || 0; const eventTone = eventGap < 0 ? "risk" : eventEoh <= 500 ? "watch" : "ok"; return <div className="sip-bar-month" key={item.month}><div className="sip-bar-positive"><i className="demand" style={{ height: `${Math.max(5, item.demand / maxBar * 100)}%` }}><b>{compact(item.demand)}</b></i><i className="supply" style={{ height: `${Math.max(5, item.supply / maxBar * 100)}%` }}><b>{compact(item.supply)}</b></i></div><div className="sip-zero-line" /><div className="sip-gap-bar"><i style={{ height: `${Math.max(6, Math.abs(gap) / maxBar * 68)}px` }}><b>{signedCompact(gap)}</b></i></div><strong>{monthLabel(item.month)}</strong>{eventRow && <span className={`sip-trend-fb ${eventTone}`}>{eventTone === "risk" ? "FB!" : eventTone === "watch" ? "FB~" : "FB✓"} {eventRow.row.model}</span>}</div>; })}</div><footer><span><i className="legend demand" /> Demand</span><span><i className="legend supply" /> Supply</span><span><i className="legend gap" /> Net Gap</span></footer></article>
			<article className="sip-trend-card sip-heatmap"><header><h2>SKU Gap Heatmap</h2><small>Click a cell to locate it in Matrix</small></header><div className="sip-table-scroll"><table><thead><tr><th>SKU</th>{months.map((month) => <th className={highlightedMonth === month ? "selected-month" : ""} key={month}>{monthLabel(month)}</th>)}</tr></thead><tbody>{heatRows.map(({ row, endings, first }) => <tr key={row.model}><th>{row.model}</th>{months.map((month) => { const plan = row.months[month] || { forecast: 0, supply: 0 }; const gap = plan.supply - plan.forecast; const isFirst = first === month; const tone = gap < -Math.max(100, plan.forecast * .1) ? "severe" : gap < 0 ? "warning" : "healthy"; const fbTone = gap < 0 ? "risk" : endings[month] <= 500 ? "watch" : "ok"; const item = exceptions.find((candidate) => candidate.row.model === row.model && candidate.month === month) || { type: gap < 0 ? "Gap" as const : "Watch" as const, row, month, issue: `Gap ${signedCompact(gap)}`, detail: "Trend selection" }; return <td className={`${tone} ${highlightedMonth === month ? "selected-month" : ""}`} key={month} onClick={() => onFocus(item)}><b>{signedCompact(gap)}</b>{isFirst && <span className={fbTone}>{fbTone === "risk" ? "FB!" : fbTone === "watch" ? "FB~" : "FB✓"}</span>}</td>; })}</tr>)}</tbody></table></div><footer><span className="severe">■ Severe</span><span className="warning">■ Watch</span><span className="healthy">■ Healthy</span></footer></article>
		</div>
		<article className="sip-trend-card sip-eoh"><header><h2>EOH Risk Trend <small>(Risk SKUs only)</small></h2><div>{riskRows.map((item, index) => <span key={item.row.model}><i style={{ background: colors[index] }} />{item.row.model}</span>)}</div></header>{riskRows.length ? <svg viewBox="0 0 440 185" role="img" aria-label="Ending on hand risk trend">{[0, 1, 2, 3, 4].map((tick) => { const value = chartMax - chartRange * tick / 4; const y = yFor(value); return <g key={tick}><line className="grid-line" x1="42" y1={y} x2="420" y2={y} /><text x="36" y={y + 3} textAnchor="end">{compact(Math.round(value))}</text></g>; })}<line className="axis-line" x1="42" y1="18" x2="42" y2="150" /><line className="stockout-line" x1="42" y1={yFor(0)} x2="420" y2={yFor(0)} /><text x="355" y={yFor(0) - 4}>Stockout line</text>{months.map((month, index) => <text key={month} x={xFor(index)} y="177" textAnchor="middle">{monthLabel(month)}</text>)}{riskRows.map((item, rowIndex) => { const points = months.map((month, index) => `${xFor(index)},${yFor(item.endings[month])}`).join(" "); return <g key={item.row.model}><polyline fill="none" stroke={colors[rowIndex]} strokeWidth="3" points={points} />{months.map((month, index) => { const first = item.first === month; const value = item.endings[month]; const tone = (item.row.months[month]?.supply || 0) - (item.row.months[month]?.forecast || 0) < 0 ? "risk" : value <= 500 ? "watch" : "ok"; return <g key={month}><circle cx={xFor(index)} cy={yFor(value)} r="4" fill={colors[rowIndex]} /><text x={xFor(index)} y={yFor(value) - 8} textAnchor="middle" fill={colors[rowIndex]}>{compact(value)}</text>{first && <g className={`svg-fb ${tone}`}><rect x={xFor(index) + 8} y={yFor(value) - 14} width="28" height="15" rx="3" /><text x={xFor(index) + 22} y={yFor(value) - 3} textAnchor="middle">{tone === "risk" ? "FB!" : tone === "watch" ? "FB~" : "FB✓"}</text></g>}</g>; })}</g>; })}</svg> : <p className="sip-v2-empty">No risk SKU in this view.</p>}</article>
