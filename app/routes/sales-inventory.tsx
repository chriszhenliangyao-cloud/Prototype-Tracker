...
function HistoryModal({ history, snapshots, revisionLogs, syncedMonth, onClose }: { history: HistoryRow[]; snapshots: ForecastSnapshot[]; revisionLogs: ChangeLog[]; syncedMonth: string; onClose: () => void }) {
	...
	const actualRows = history
		.filter((item) => inRange(item.month) && modelMatches(item.model) && (category === "all" || item.category === category))
		.sort((a, b) => a.model.localeCompare(b.model) || a.month.localeCompare(b.month) || a.product.localeCompare(b.product));
	const snapshotRows = snapshots
		.filter((item) => inRange(item.archiveMonth) && modelMatches(item.model) && (category === "all" || item.category === category))
		.sort((a, b) => a.model.localeCompare(b.model) || a.archiveMonth.localeCompare(b.archiveMonth) || a.forecastMonth.localeCompare(b.forecastMonth));
	const logRows = revisionLogs
		.filter((item) => inRange(item.time === "Now" ? item.month : item.time.slice(0, 7)) && modelMatches(item.model) && (category === "all" || categoryFor(item.model) === category))
		.sort((a, b) => a.model.localeCompare(b.model) || (b.time === "Now" ? "9999" : b.time).localeCompare(a.time === "Now" ? "9999" : a.time) || a.month.localeCompare(b.month) || a.field.localeCompare(b.field));
...
function TrendView({ rows, months, totals, exceptions, highlightedMonth, onFocus }: { rows: PlanningRow[]; months: string[]; totals: Array<{ month: string; demand: number; supply: number }>; exceptions: ExceptionItem[]; highlightedMonth: string; onFocus: (item: ExceptionItem) => void }) {
	const positivePeak = Math.max(1, ...totals.flatMap((item) => [item.demand, item.supply]));
	const negativePeak = Math.max(1, ...totals.map((item) => Math.max(0, item.demand - item.supply)));
	const roundedAxis = (value: number) => {
	...
	const positiveAxisMax = roundedAxis(positivePeak);
	const negativeAxisMax = roundedAxis(negativePeak);
	...
	const xFor = (index: number) => 72 + index * 146;
	const yFor = (value: number) => 18 + ((chartMax - Math.max(chartMin, Math.min(chartMax, value))) / chartRange) * 132;
	const colors = ["#dc2626", "#2563eb", "#079669", "#f97316"];
	const valueLabelOffsets = [14, -8, -28, 24];
	...
	return <section className="sip-trend">
		<div className="sip-trend-top">
			<article className="sip-trend-card sip-demand-chart"><header><h2>Monthly Demand vs Supply <span className="sip-info" title="Demand, supply and net gap by month">i</span></h2></header><footer><span><i className="legend demand" /> Demand</span><span><i className="legend supply" /> Supply</span><span><i className="legend gap" /> Net Gap (S − D)</span></footer><div className="sip-bars" role="img" aria-label="Monthly demand, supply and net gap chart"><div className="sip-bar-y-axis"><b>{compact(positiveAxisMax)}</b><b>{compact(Math.round(positiveAxisMax / 2))}</b><b>0</b><b>−{compact(Math.round(negativeAxisMax / 2))}</b><b>−{compact(negativeAxisMax)}</b></div>{totals.map((item) => { const gap = item.supply - item.demand; const eventRow = heatRows.find(({ first }) => first === item.month); const eventGap = eventRow ? (eventRow.row.months[item.month]?.supply || 0) - (eventRow.row.months[item.month]?.forecast || 0) : 0; const eventEoh = eventRow?.endings[item.month] || 0; const eventTone = eventGap < 0 ? "risk" : eventEoh <= 500 ? "watch" : "ok"; return <div className="sip-bar-month" key={item.month}><div className="sip-bar-plot"><div className="sip-bar-positive"><i className="demand" style={{ height: `${item.demand === 0 ? 0 : Math.max(3, item.demand / positiveAxisMax * 100)}%` }}><b>{compact(item.demand)}</b></i><i className="supply" style={{ height: `${item.supply === 0 ? 0 : Math.max(3, item.supply / positiveAxisMax * 100)}%` }}><b>{compact(item.supply)}</b></i></div><div className="sip-gap-bar"><i style={{ height: `${gap === 0 ? 0 : Math.max(5, Math.abs(Math.min(0, gap)) / negativeAxisMax * 100)}%` }}><b>{signedCompact(gap)}</b></i></div></div><div className="sip-zero-line" /><strong>{monthLabel(item.month)}</strong>{eventRow && <span className={`sip-trend-fb ${eventTone}`}>{eventTone === "risk" ? "FB!" : eventTone === "watch" ? "FB~" : "FB✓"} {eventRow.row.model}</span>}</div>; })}</div></article>
		...
		<article className="sip-trend-card sip-eoh"><header><h2>EOH Risk Trend <small>(Risk SKUs only)</small> <span className="sip-info" title="Ending on hand for risk SKUs only">i</span></h2><div>{riskRows.map((item, index) => <span key={item.row.model}><i style={{ background: colors[index] }} />{item.row.model}</span>)}</div></header>{riskRows.length ? <div className="sip-eoh-plot"><b>EOH (Units)</b><svg viewBox="0 0 470 185" role="img" aria-label="Ending on hand risk trend from negative four thousand to positive four thousand units">{[4000, 2000, 0, -2000, -4000].map((value) => { const y = yFor(value); return <g key={value}><line className="grid-line" x1="42" y1={y} x2="390" y2={y} /><text x="36" y={y + 3} textAnchor="end">{compact(value)}</text></g>; })}<line className="axis-line" x1="42" y1="18" x2="42" y2="150" /><line className="axis-line" x1="42" y1="150" x2="390" y2="150" /><line className="stockout-line" x1="42" y1={yFor(0)} x2="390" y2={yFor(0)} /><text className="stockout-label" x="400" y={yFor(0) + 3} textAnchor="start">Stockout line</text>{months.map((month, index) => <text key={month} x={xFor(index)} y="177" textAnchor="middle">{monthLabel(month)}</text>)}{riskRows.map((item, rowIndex) => { const points = months.map((month, index) => `${xFor(index)},${yFor(item.endings[month])}`).join(" "); return <g key={item.row.model}><polyline className="risk-series-line" fill="none" stroke={colors[rowIndex]} points={points} />{months.map((month, index) => { const first = item.first === month; const value = item.endings[month]; const tone = (item.row.months[month]?.supply || 0) - (item.row.months[month]?.forecast || 0) < 0 ? "risk" : value <= 500 ? "watch" : "ok"; return <g key={month}><circle className="risk-series-point" cx={xFor(index)} cy={yFor(value)} r="3.25" fill={colors[rowIndex]} /><text className="point-value" x={xFor(index)} y={yFor(value) + valueLabelOffsets[rowIndex]} textAnchor="middle" style={{ fill: colors[rowIndex] }}>{compact(value)}</text>{first && <g className={`svg-fb ${tone}`}><rect x={Math.min(352, xFor(index) + 10)} y={yFor(value) - 13} width="28" height="15" rx="3" /><text x={Math.min(366, xFor(index) + 24)} y={yFor(value) - 2} textAnchor="middle">{tone === "risk" ? "FB!" : tone === "watch" ? "FB~" : "FB✓"}</text></g>}</g>; })}</g>; })}</svg></div> : <p className="sip-v2-empty">No risk SKU in this view.</p>}</article>
...
export default function SalesInventory() {
	...
	const [trendFromMonth, setTrendFromMonth] = useState(initialPlanningMonths[0]);
	const [trendToMonth, setTrendToMonth] = useState(initialPlanningMonths.at(-1)!);
	...
	useEffect(() => { setTrendFromMonth(months[0]); setTrendToMonth(months.at(-1)!); }, [months]);

	const categories = [...new Set(rows.map((row) => row.category))].sort();
	const planningMonths = [...new Set(rows.flatMap((row) => Object.keys(row.months)))].sort();
	const trendMonths = planningMonths.filter((month) => month >= trendFromMonth && month <= trendToMonth);
	...
	const stockouts = filtered.filter((row) => Object.values(calculateEndings(row, activeMonths)).some((ending) => ending < 0)).length;
	const exceptions: ExceptionItem[] = filtered.flatMap((row) => {
		const life = lifecycleOf(row, activeMonths[0]); const endings = calculateEndings(row, activeMonths); const first = life === "New" ? firstBatchMonth(row, activeMonths) : null;
		return activeMonths.flatMap<ExceptionItem>((month): ExceptionItem[] => { const plan = row.months[month] || { forecast: 0, supply: 0 }; const gap = plan.supply - plan.forecast; const eoh = endings[month];
	...
	const undoClosing = () => {
		...
		if (!closed || closed < initialPlanningMonths[0]) return;
		const closedHistory = history.filter((item) => item.month === closed);
		setRows((current) => current.map((row) => ({
			...row,
			inventory: closedHistory.find((item) => item.model === row.model)?.beginningInventory ?? row.inventory,
		})));
		setDraftRows(null);
		setMonths([closed, ...months.slice(0, 2)]);
		setHistory((current) => current.filter((item) => item.month !== closed));
		setSnapshots((current) => {
			const restored = current.filter((item) => item.archiveMonth !== closed);
			setRevisionLogs(snapshotRevisionLogs(restored));
			return restored;
		});
		setLastClosedMonth(lastMonth(closed));
	};
	...
	return <div className="sip-app sip-v2-app"><ModeHeader theme={theme} onToggleTheme={() => { const value = theme === "dark" ? "light" : "dark"; setTheme(value); document.documentElement.setAttribute("data-theme", value); }} /><main className="sip-v2-main">
		<header className="sip-v2-toolbar"><h1>Sales &amp; Inventory{view === "Trend" ? " · Trend Analysis" : ""}</h1><div className="sip-v2-tools"><button className="sip-btn" onClick={() => setHistoryOpen(true)}>↻ Pull History</button><span className="sip-v2-synced">History synced: {monthLabel(lastClosedMonth || "2026-07")} ✓</span><ModelFilter rows={rows} selected={models} onChange={setModels} /><label className="sip-v2-field"><span>Category</span><select value={category} onChange={(event) => setCategory(event.target.value)}><option value="all">All Categories</option>{categories.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><div className="sip-v2-field"><span>Lifecycle</span><div className="sip-v2-segment">{(["All", "New", "Launched"] as const).map((item) => <button className={lifecycle === item ? "active" : ""} onClick={() => setLifecycle(item)} key={item}>{item}</button>)}</div></div>{view === "Trend" ? <div className="sip-v2-trend-period"><label className="sip-v2-field"><span>From</span><select value={trendFromMonth} onChange={(event) => { const value = event.target.value; setTrendFromMonth(value); if (value > trendToMonth) setTrendToMonth(value); }}>{planningMonths.map((month) => <option key={month} value={month}>{monthLabel(month)}</option>)}</select></label><label className="sip-v2-field"><span>To</span><select value={trendToMonth} onChange={(event) => { const value = event.target.value; setTrendToMonth(value); if (value < trendFromMonth) setTrendFromMonth(value); }}>{planningMonths.map((month) => <option key={month} value={month}>{monthLabel(month)}</option>)}</select></label></div> : <div className="sip-v2-field sip-v2-period"><span>Period</span><strong>{shortMonth(months[0])} – {shortMonth(months.at(-1)!)} {months[0].slice(0, 4)}</strong></div>}<Toggle checked={onlyGap} label="Only Gap" onChange={setOnlyGap} /><Toggle checked={firstBatchOnly} label="First Batch Only" onChange={setFirstBatchOnly} />{view === "Matrix" && <><button className={`sip-btn ${draftRows ? "has-draft" : ""}`} onClick={() => setEditorOpen(true)}>Edit Plan{draftRows ? " · Draft saved" : ""}</button><button className="sip-btn" onClick={() => setArchiveOpen(true)}>Forecast Archive</button></>}<button className="sip-btn warning" disabled={!lastClosedMonth || lastClosedMonth < initialPlanningMonths[0]} onClick={undoClosing}>Undo Closing</button><button className="sip-btn primary" onClick={() => setClosingOpen(true)}>Month Closing</button><div className="sip-v2-field"><span>View</span><div className="sip-v2-segment sip-v2-view-switch">{(["Matrix", "Trend"] as const).map((item) => <button className={view === item ? "active" : ""} onClick={() => setView(item)} key={item}>{item}</button>)}</div></div></div></header>

		<section className="sip-v2-kpis"><article><span>{activeMonths.length}M DEMAND</span><strong>{compact(demand)}</strong><small>{shortMonth(activeMonths[0])} – {shortMonth(activeMonths.at(-1)!)}</small><b className="blue">⌁</b></article><article><span>{activeMonths.length}M SUPPLY</span><strong>{compact(supply)}</strong><small>{shortMonth(activeMonths[0])} – {shortMonth(activeMonths.at(-1)!)}</small><b className="green">◇</b></article><article className={supply - demand < 0 ? "danger" : ""}><span>NET GAP</span><strong>{signedCompact(supply - demand)}</strong><small>Supply − Demand</small></article><article className={`worst ${(worst?.supply || 0) - (worst?.demand || 0) < 0 ? "danger" : ""}`} role="button" tabIndex={0} onClick={() => { if (worst) { setView("Trend"); setHighlightedMonth(worst.month); } }}><span>WORST MONTH</span><strong>{worst ? monthLabel(worst.month) : "—"}</strong><em>{worst ? signedCompact(worst.supply - worst.demand) : "—"}</em><small>Gap · click to highlight</small></article><article className={stockouts ? "danger stockout" : "stockout"}><span>STOCKOUT SKU</span><strong>{stockouts}</strong><small>Any in selected period</small><b className="alert" aria-label="Attention">!</b></article></section>
