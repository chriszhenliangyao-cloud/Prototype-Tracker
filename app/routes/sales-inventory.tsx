...
function HistoryModal({ history, snapshots, revisionLogs, syncedMonth, onClose }: { history: HistoryRow[]; snapshots: ForecastSnapshot[]; revisionLogs: ChangeLog[]; syncedMonth: string; onClose: () => void }) {
	...
	const months = [...new Set([...history.map((item) => item.month), ...snapshots.map((item) => item.archiveMonth), syncedMonth])].sort().reverse();
	const actualRows = month === "all" ? history : history.filter((item) => item.month === month);
	const snapshotRows = month === "all" ? snapshots : snapshots.filter((item) => item.archiveMonth === month);
	const logRows = month === "all" ? revisionLogs : revisionLogs.filter((item) => item.time.startsWith(month) || item.month === month);
	return <div className="sip-overlay"><section className="sip-modal wide sip-v2-history" role="dialog" aria-modal="true"><header><div><h2>Pull History</h2><p>Read-only actuals, inventory, forecast snapshots and revision history for all closed months.</p></div><button onClick={onClose}>×</button></header><div className="sip-v2-history-filter"><label>Month<select value={month} onChange={(event) => setMonth(event.target.value)}><option value="all">All Months</option>{months.map((item) => <option key={item} value={item}>{monthLabel(item)}</option>)}</select></label><span>History synced: {monthLabel(syncedMonth)} ✓</span></div><nav className="sip-v2-tabs"><button className={tab === "actuals" ? "active" : ""} onClick={() => setTab("actuals")}>Monthly Actuals</button><button className={tab === "snapshots" ? "active" : ""} onClick={() => setTab("snapshots")}>Forecast Snapshots</button><button className={tab === "revisions" ? "active" : ""} onClick={() => setTab("revisions")}>Revision Log</button></nav>
	{tab === "actuals" && <div className="sip-table-scroll"><table><thead><tr><th>Month</th><th>Model</th><th>Product</th><th>Actual Shipment</th><th>Actual Supply</th><th>Beginning Inventory</th><th>Ending Inventory</th></tr></thead><tbody>{actualRows.length ? actualRows.map((item, index) => <tr key={`${item.month}-${item.model}-${index}`}><td>{monthLabel(item.month)}</td><td>{item.model}</td><td>{item.product}</td><td>{formatNumber(item.actualSales)}</td><td>{formatNumber(item.actualSupply)}</td><td>{formatNumber(item.beginningInventory)}</td><td>{formatNumber(item.endingInventory)}</td></tr>) : <tr><td colSpan={7}>No mock actuals for this month.</td></tr>}</tbody></table></div>}
...
function PlanEditor({ rows, months, snapshots, revisionLogs, onClose, onSave }: { rows: PlanningRow[]; months: string[]; snapshots: ForecastSnapshot[]; revisionLogs: ChangeLog[]; onClose: () => void; onSave: (rows: PlanningRow[], publish: boolean, logs: ChangeLog[]) => void }) {
	...
	return <div className="sip-overlay"><section className="sip-modal wide sip-v2-editor" role="dialog" aria-modal="true"><header><div><h2>Rolling Plan Editor</h2><p>Edit rolling monthly forecast and supply plan</p></div><button onClick={onClose}>×</button></header><div className="sip-v2-editor-meta"><div><span>▣ Plan Cycle: {monthLabel(months[0])}</span><span>▣ Window: {shortMonth(months[0])} – {shortMonth(months.at(-1)!)} {months[0].slice(0, 4)}</span><span className="autosaved">◉ Autosaved 2 min ago</span></div><div><b>Draft v4</b><small>Based on {monthLabel(lastMonth(months[0]))} archive</small></div></div><nav className="sip-v2-tabs"><button className={tab === "grid" ? "active" : ""} onClick={() => setTab("grid")}>Grid Input</button><button className={tab === "versions" ? "active" : ""} onClick={() => setTab("versions")}>Version History</button><button className={tab === "log" ? "active" : ""} onClick={() => setTab("log")}>Change Log ({logs.length})</button></nav>
		...
		{tab === "versions" && <div className="sip-v2-version-list"><article><b>Draft v4</b><span>Current editable draft · Now</span><button className="sip-btn" onClick={() => setCompareArchive(archiveMonths[0] || null)}>Compare</button></article>{archiveMonths.map((archiveMonth, index) => <article key={archiveMonth}><b>Plan Snapshot v{archiveMonths.length - index + 1}</b><span>Published by Ivy · {monthLabel(archiveMonth)}</span><div><button className="sip-btn" onClick={() => restoreArchive(archiveMonth)}>Restore as Draft</button><button className="sip-btn" onClick={() => { setViewLogArchive(archiveMonth); setTab("log"); }}>View Log</button></div></article>)}{compareArchive && <section className="sip-v2-compare"><header><b>Draft v4 vs. {monthLabel(compareArchive)} Snapshot</b><button onClick={() => setCompareArchive(null)}>×</button></header><div className="sip-table-scroll"><table><thead><tr><th>SKU</th><th>Month</th><th>Forecast Change</th><th>Supply Change</th></tr></thead><tbody>{comparisonRows.length ? comparisonRows.map((item) => <tr key={`${item.model}-${item.month}`}><td>{item.model}</td><td>{monthLabel(item.month)}</td><td>{signedCompact(item.forecastChange)}</td><td>{signedCompact(item.supplyChange)}</td></tr>) : <tr><td colSpan={4}>No differences from this snapshot.</td></tr>}</tbody></table></div></section>}</div>}
		{tab === "log" && <div className="sip-table-scroll"><table><thead><tr><th>SKU</th><th>Month</th><th>Field</th><th>Before</th><th>After</th><th>User</th><th>Time</th></tr></thead><tbody>{[...logs, ...revisionLogs.filter((item) => !viewLogArchive || item.time.startsWith(viewLogArchive))].length ? [...logs, ...revisionLogs.filter((item) => !viewLogArchive || item.time.startsWith(viewLogArchive))].map((item, index) => <tr key={`${item.model}-${item.month}-${item.field}-${index}`}><td>{item.model}</td><td>{monthLabel(item.month)}</td><td>{item.field}</td><td>{formatNumber(item.before)}</td><td>{formatNumber(item.after)}</td><td>{item.user}</td><td>{item.time === "Now" ? item.time : new Date(item.time).toLocaleString()}</td></tr>) : <tr><td colSpan={7}>No changes in this version.</td></tr>}</tbody></table></div>}
