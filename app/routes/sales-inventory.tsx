...
function TrendView({ rows, months, totals, exceptions, highlightedMonth, onFocus }: { rows: PlanningRow[]; months: string[]; totals: Array<{ month: string; demand: number; supply: number }>; exceptions: ExceptionItem[]; highlightedMonth: string; onFocus: (item: ExceptionItem) => void }) {
	...
	return <section className="sip-trend">
		<div className="sip-trend-top">
			...
			<article className="sip-trend-card sip-heatmap"><header><h2>SKU Gap Heatmap</h2><small>Click a cell to locate it in Matrix</small></header><div className="sip-table-scroll"><table><thead><tr><th>SKU</th>{months.map((month) => <th className={highlightedMonth === month ? "selected-month" : ""} key={month}>{monthLabel(month)}</th>)}</tr></thead><tbody>{heatRows.map(({ row, endings, first }) => <tr key={row.model}><th>{row.model}</th>{months.map((month) => { const plan = row.months[month] || { forecast: 0, supply: 0 }; const gap = plan.supply - plan.forecast; const isFirst = first === month; const tone = gap < -Math.max(100, plan.forecast * .1) ? "severe" : gap < 0 ? "warning" : "healthy"; const item = exceptions.find((candidate) => candidate.row.model === row.model && candidate.month === month) || { type: gap < 0 ? "Gap" as const : "Watch" as const, row, month, issue: `Gap ${signedCompact(gap)}`, detail: "Trend selection" }; return <td className={`${tone} ${highlightedMonth === month ? "selected-month" : ""}`} key={month} onClick={() => onFocus(item)}><b>{signedCompact(gap)}</b>{isFirst && <span>{gap < 0 ? "FB!" : endings[month] <= 500 ? "FB~" : "FB✓"}</span>}</td>; })}</tr>)}</tbody></table></div><footer><span className="severe">■ Severe</span><span className="warning">■ Watch</span><span className="healthy">■ Healthy</span></footer></article>
		</div>
		<article className="sip-trend-card sip-eoh"><header><h2>EOH Risk Trend <small>(Risk SKUs only)</small></h2><div>{riskRows.map((item, index) => <span key={item.row.model}><i style={{ background: colors[index] }} />{item.row.model}</span>)}</div></header>{riskRows.length ? <svg viewBox="0 0 440 185" role="img" aria-label="Ending on hand risk trend"><line className="stockout-line" x1="42" y1={yFor(0)} x2="420" y2={yFor(0)} /><text x="355" y={yFor(0) - 4}>Stockout line</text>{months.map((month, index) => <text key={month} x={xFor(index)} y="177" textAnchor="middle">{shortMonth(month)}</text>)}{riskRows.map((item, rowIndex) => { const points = months.map((month, index) => `${xFor(index)},${yFor(item.endings[month])}`).join(" "); return <g key={item.row.model}><polyline fill="none" stroke={colors[rowIndex]} strokeWidth="3" points={points} />{months.map((month, index) => <g key={month}><circle cx={xFor(index)} cy={yFor(item.endings[month])} r="4" fill={colors[rowIndex]} /><text x={xFor(index)} y={yFor(item.endings[month]) - 8} textAnchor="middle" fill={colors[rowIndex]}>{compact(item.endings[month])}</text></g>)}</g>; })}</svg> : <p className="sip-v2-empty">No risk SKU in this view.</p>}</article>
