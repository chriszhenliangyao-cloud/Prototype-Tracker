...
function HistoryModal({ history, snapshots, revisionLogs, syncedMonth, onClose }: { history: HistoryRow[]; snapshots: ForecastSnapshot[]; revisionLogs: ChangeLog[]; syncedMonth: string; onClose: () => void }) {
	...
	const modelMatches = (model: string) => model.toLowerCase().includes(modelQuery.trim().toLowerCase());
	const categoryFor = (model: string) => history.find((item) => item.model === model)?.category || snapshots.find((item) => item.model === model)?.category || "";
	const actualRows = history.filter((item) => (month === "all" || item.month === month) && modelMatches(item.model) && (category === "all" || item.category === category));
	const snapshotRows = snapshots.filter((item) => (month === "all" || item.archiveMonth === month) && modelMatches(item.model) && (category === "all" || item.category === category));
	const logRows = revisionLogs.filter((item) => (month === "all" || item.time.startsWith(month) || item.month === month) && modelMatches(item.model) && (category === "all" || categoryFor(item.model) === category));
	return <div className="sip-overlay"><section className="sip-modal wide sip-v2-history" role="dialog" aria-modal="true"><header><div><h2>Pull History</h2><p>Read-only actuals, inventory, forecast snapshots and revision history for all closed months.</p></div><button onClick={onClose}>×</button></header><div className="sip-v2-history-filter"><label>Month<select value={month} onChange={(event) => setMonth(event.target.value)}><option value="all">All Months</option>{months.map((item) => <option key={item} value={item}>{monthLabel(item)}</option>)}</select></label><label>Model<input type="search" placeholder="Search model" value={modelQuery} onChange={(event) => setModelQuery(event.target.value)} /></label><label>Category<select value={category} onChange={(event) => setCategory(event.target.value)}><option value="all">All Categories</option>{categories.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><span>History synced: {monthLabel(syncedMonth)} ✓</span></div><nav className="sip-v2-tabs"><button className={tab === "actuals" ? "active" : ""} onClick={() => setTab("actuals")}>Monthly Actuals</button><button className={tab === "snapshots" ? "active" : ""} onClick={() => setTab("snapshots")}>Forecast Snapshots</button><button className={tab === "revisions" ? "active" : ""} onClick={() => setTab("revisions")}>Revision Log</button></nav>
