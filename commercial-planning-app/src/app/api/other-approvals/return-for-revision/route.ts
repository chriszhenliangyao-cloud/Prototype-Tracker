...
import { returnOtherApprovalForRevision } from "@/lib/otherApprovals";
...
export async function POST(request: NextRequest) {
  ...
  const payload = (await request.json()) as { id?: unknown; note?: unknown };
  const { accessibleCountryCodes, approvalCapabilities } =
    await getOtherApprovalApiAccess(session);
  const result = await returnOtherApprovalForRevision({
  ...
  return NextResponse.json({ ...result, message: "Returned to the requester for revision." });
}
